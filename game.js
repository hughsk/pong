...
Pong.prototype.connect = function(host) {
	...
	this.server.once('ready', function(data) {
		console.log('Found another player, should be ready to go!');

		data = data || {};
		self.options.width = data.width || self.options.width;
		self.options.height = data.height || self.options.height;
		self.theirPaddle.x = self.options.width;
